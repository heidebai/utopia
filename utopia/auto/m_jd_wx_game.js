...
if (mode) {
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10084&activityId=1779714416015704065&templateId=831049284&nodeId=101001&prd=cjwx'
    ...
    async exec() {
        ...
        if (this.isV1Act) {
            let gameInfo = await this.api('/api/game/getGameInfo',
                {"gameUrl": `https://lzkj-yc.isvjd.com/index.html?templateId=${$.templateId}&token=${this.isvToken}`, "shareUserId": ""});
            ...
            for (let i = 0; i < gameInfo.data.gameChance; i++) {
                try {
                    ...
                    const encrypted = this.rsaEncrypt(init.data.publicKey, {encryptionScheme: 'pkcs1'},
        ...
        let activityContent = await this.api('wxGameActivity/activityContent',
            `activityId=${$.activityId}&pin=${this.Pin}`);
        ...
        for (let i = 0; i < Math.min(drawCount, 50); i++) {
            ...
            let prize = await this.api('wxGameActivity/gameOverRecord',
                `activityId=${$.activityId}&pin=${this.Pin}&score=${score}`);
            if (prize.result) {
