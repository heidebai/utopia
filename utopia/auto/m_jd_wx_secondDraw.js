...
if (mode) {
    ...
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10035&templateId=2021062190900dmpss011&activityId=1729812465189593089&nodeId=101001&prd=cjwx&shopid=1000015502'
}
$.version = "v1.0.0"
...
class Task extends Env {
    ...
    async exec() {
        ...
        let game = await this.api('wxSecond/getTaskDay',
            `activityId=${$.activityId}&pin=${this.Pin}&uuid=${uuid}`);
        if (game.result) {
            for (const ele of game.data || []) {
                let taskType = ele.taskType
                for (let i = 0; i < ele.dayMaxNumber && ele.finishNumber === 0;
                    i++) {
                    if ([2, 5].includes(taskType)) {
                        let goods = ele.activityTaskGoods.slice(i * ele.commodity,
                            i * ele.commodity + ele.commodity).filter(
                            o => o.complete === 0)
        ...
        game = await this.api('wxSecond/getTask',
            `activityId=${$.activityId}&pin=${this.Pin}&uuid=${uuid}`);
        if (game.result) {
            for (const ele of game.data || []) {
                let taskType = ele.taskType
                for (let i = 0; i < ele.dayMaxNumber && ele.finishNumber === 0;
                    ...
                    if ([12].includes(taskType)) {
                        let t = await this.api(
                            'wxSecond/finishTask',
                            `activityId=${$.activityId}&uuid=${uuid}&taskType=${taskType}&skuId=`);
                        if (t.result) {
                            count += t.data.score
                        }
                    }
        ...
        for (let i = 0; i < Math.min(count, 10); i++) {
            let auth = await this.api('/wxSecond/checkAuth',
                `activityId=${$.activityId}&brushBane=${brushBane}&bid=${bid}&pin=${this.Pin}`);
            let brushResult = encodeURIComponent(auth.data.data.brushResult)
            let prize = await this.api('wxSecond/start',
                `activityId=${$.activityId}&uuid=${uuid}&seconds=${targetTime}&brushBane=${brushResult}&bid=${encodeURIComponent(
                    auth.data.data.bid)}`);
            if (prize.result) {
                let msg = prize.data.draw.drawOk ? prize.data.draw.name
        ...
        let records = await this.api('wxSecond/myPrize',
            `activityId=${$.activityId}&uuid=${uuid}`);
        if (records.result) {
            for (let record of
            records?.data?.filter(o => o.type === 7 && o.needWriteAddress === 'y'
