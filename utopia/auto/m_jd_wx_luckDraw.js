...
if (mode) {
    ...
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interaction/v1/index?activityType=10041&activityId=1760596946454425601&templateId=20210714190900ysz01&nodeId=101001&prd=crm'
    $.activityUrl = 'https://jingyun-rc.isvjcloud.com/h5/pages/bestTicket/default?id=caf6fb4bbad5d0fbcf67fba745da3fec&userId=139755&sid=&un_area=1_72_55674_0'
    $.activityUrl = "https://cjhy-isv.isvjcloud.com/wxDrawActivity/activity?activityId=7c08681e9f9946729df0fb6023afb223"
    $.activityUrl = "https://lorealjdcampaign-rc.isvjcloud.com/interact/index?activityType=10021&activityId=1778698973209247746&templateId=20210527190900dazhuanpan01&nodeId=101001&prd=crm"
    $.activityUrl = "https://jingyun-rc.isvjcloud.com/h5/pages/turntable/turntable?id=7d089ffc2d54fa0a29459b662972d547&userId=1000000289&actForm=single"
    $.activityUrl = "https://cjhy-isv.isvjcloud.com/wxDrawActivity/activity/activity?activityId=2e92b6a3afb4490fb48c922b1780150a"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10021&activityId=1792456979906080770&templateId=33ed5a4c-5aae-4b99-8b92-85c1c2e5a785&nodeId=101001&prd=cjwx"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10021&activityId=1790722771933700098&templateId=33ed5a4c-5aae-4b99-8b92-85c1c2e5a785&nodeId=101001&prd=cjwx"
    ...
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10021&activityId=1788766433527828482&templateId=2023120710021dzpcj01&nodeId=101001&prd=cjwx"
...
class Task extends Env {
    ...
    async exec() {
        ...
        let context = await this.login(
            {fn: this.isJinggengAct ? `ql/front/${jinggengFn}` : `wuxian/user/getLottery/${$.activityId}`});
        ...
        if (this.isHzbzAct) {
            ...
            /**
             ...
             {"prizeId":32435,"prizeName":"200积分","needInputInfo":false,"drawChances":1,"canNotDraw":false,"buyerFrom":1,"code":2,"txt":"中奖了"}
             */
        ...
        if (this.isV2Act) {
            //https://lzkj-isv.isvjcloud.com/prod/cc/interaction/v2/api/30003/getPoints
            ...
            // let userPrizes = await this.api(`/api/${this.activityType}/userPrizes`, {});
            // $.log(userPrizes)
            // if (userPrizes?.code == 200) {
            //     for (let record of userPrizes?.data?.filter(o => o.prizeType === 3)) {
            //         this.addressId = record.userPrizeId
            //         this.prizeId = record.activityPrizeId
            //         this.prizeName = record.prizeName
            //         await this.saveAddress()
            //     }
            // }
        ...
        // M幸运抽奖 兼容 jinggeng loadBlindBox
        if (this.isJinggengAct) {
            if ($.activityUrl.includes("loadBlindBox")) {
                ...
                // buy 购物赠送 cartItem 加购商品赠送
                // focus 关注店铺赠送 everyDay 每日赠送 pointExchange 积分兑换
                ...
                if (tasks.length > 0) {
                    for (let i = 0; i < tasks.length; i++) {
                        ...
                        const taskType = element.taskType
                        const data = await this.api(`ql/front/postDrawTimes`,
                ...
                if (data.status !== "1") {
                    ...
                    if (m === 1 && data.status === "-3") {
                        ...
                        await this.api(`wuxian/user/flowShop/${$.shopId}/${$.venderId}`,
                            {'shopId': $.shopId, 'source': "01", 'token': this.isvToken, 'venderId': $.venderId});
                        continue
                    }
                }
                if (data?.winId) {
                ...
                try {
                    if (follow && myInfo.data.follow.hasFollowTimes < myInfo.data.follow.followTimes) {
                        ...
                        for (let i = 0; i < count; i++) {
                            ...
                            let follow = await this.api('wxDrawActivity/follow',
                                `activityId=${$.activityId}&pin=${this.Pin}&skuId=${skuId}`)
                            if (!follow.result) {
                                break
                            }
                        }
        ...
        if ([26, 124, 125, 128, 129].includes($.activityType)) {
            let check = await this.api("common/joinConfig/check",
