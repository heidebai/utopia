...
if (mode) {
    ...
    $.activityUrl = 'https://jinggeng-rc.isvjcloud.com/ql/front/floor?id=9e8080358d7a2559018d7d78d8dc3eff&user_id=1000000331&sid=&un_area=1_72_55674_0'
}
    ...
    async exec() {
        ...
        if (this.activityUrl.includes("jinggeng")) {
            for (let i = 0; i < 100; i++) {
                ...
                let data = await this.api('/ql/front/ajaxFloor',
                    `act_id=${$.activityId}&user_id=${$.userId}&buyer_logo=${encodeURIComponent(
                        this.attrTouXiang)}&comment_info=${encodeURIComponent(pinyu)}`);
        ...
        let count = 0
        while (count < 10) {
            // let currentFloor = await this.api('wxBuildActivity/currentFloor', `activityId=${$.activityId}`);
            // this.log(`当前楼层：${currentFloor.data?.currentFloors}`)
            let prize = await this.api('wxBuildActivity/publish',
