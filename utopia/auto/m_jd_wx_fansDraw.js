...
if (mode) {
    $.activityUrl = 'https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/d0a4535d3d6e46ed81ebc514aeb6abe1?activityId=d0a4535d3d6e46ed81ebc514aeb6abe1'
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10037&activityId=1791103560813043713&templateId=20210621090000fshd001&nodeId=101001&prd=cjwx'
    $.maxCookie = 5
}
$.version = "v1.0.0"
...
class Task extends Env {
    ...
    async exec() {
        ...
        let taskIds = activityContent.data.actInfo.taskIds //游戏
        ...
        // 1 签到 2 浏览商品 3 加购 4分享 5提醒 6 领券  7 未知
        let taskMap = {
            '1': 'task1Sign',
            '2': 'task2BrowGoods',
            '3': 'task3AddCart',
            ...
            for (let i = 1; i <= task.upLimit - task.finishedCount; i++) {
                try {
                    ...
                    if (taskMap[ele] === 'task2BrowGoods' && task.taskGoodList?.length > 0) {
                        ...
                        await this.api(`wxFansInterActionActivity/doBrowGoodsTask`,
                            `activityId=${$.activityId}&uuid=${uuid}&skuId=${skuId}`);
                    }
                    if (taskMap[ele] === 'task3AddCart' && task.taskGoodList?.length > 0) {
                        let taskGoodList = task.taskGoodList;
                        //任务
                        let skuId = taskGoodList[i].skuId;
                        await this.api(`wxFansInterActionActivity/doAddGoodsTask`,
                            `activityId=${$.activityId}&uuid=${uuid}&skuId=${skuId}`);
                    }
                    ...
                    if (taskMap[ele] === 'task6GetCoupon' && task.taskCouponInfoList?.length > 0) {
                        ...
                        await this.api(`wxFansInterActionActivity/doGetCouponTask`,
                            `activityId=${$.activityId}&uuid=${uuid}&couponId=${couponId}`);
                    }
        ...
        let energyValue = actorInfo.energyValue;//积分
        energyValue += actorInfo.fansLoveValue
        let prizeOneStatus = actorInfo.prizeOneStatus;//积分
        let prizeTwoStatus = actorInfo.prizeTwoStatus;//积分
        let prizeThreeStatus = actorInfo.prizeThreeStatus;//积分
        ...
        if (drawType) {
            let prize = await this.api(`wxFansInterActionActivity/startDraw`,
                `activityId=${$.activityId}&uuid=${uuid}&drawType=${drawType}`);
        ...
        // //自动填地址
        // let recordData = await this.api('wxFansInterActionActivity/getDrawRecord',
        //     `uuid=${actorInfo.uuid}&activityId=${$.activityId}`);
        // if (recordData?.result) {
        //     for (let record of recordData?.data?.filter(
        //         o => o.type === 7 && o.needWriteAddress === 'y'
        //             && o.drawTime === $.now('yyyy-MM-dd'))) {
