...
const $ = new Env('M店铺签到');
$.shopToken = decodeURIComponent(
    process.argv.splice(2)?.[0] || process.env.M_TOKEN_SHOP_SIGN);
$.concNum = 1
if (mode) {
    $.maxCookie = 5
}
...
class Task extends Env {
    ...
    async exec() {
        if (!$.data) {
            let data = await this.api(`https://api.m.jd.com/api`, {
                "appId": "4da33",
        ...
        $.id = $.data.data.id
        let re = await this.api('https://api.m.jd.com/api', {
            "appId": "4da33",
