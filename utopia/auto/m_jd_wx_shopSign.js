...
if (mode) {
    ...
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10002&activityId=1773625195793281026&templateId=20201228083300lxqdsl011&nodeId=101001003&prd=cjwx&day=25'
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10040&activityId=1773274338878369794&templateId=20210518190900qrqd011&nodeId=101001&prd=cjwx'
    $.activityUrl = 'https://lorealjdcampaign-rc.isvjcloud.com/interact/index?activityType=10023&activityId=1770900388381171714&templateId=20210518190900rlqd01&nodeId=101001&prd=crm'
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10003&activityId=1775459909728444418&templateId=8c7bb445-deab-42cf-922a-b585065f298f&nodeId=101001001&prd=cjwx'
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interaction/v2/10001/1001/?shopId=1000310642&activityId=1787146044937420802&shareId=1787372228120666113'
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10023&activityId=1773666244126314498&templateId=2023110801rlqd&nodeId=101001&prd=cjwx&adsource=tg_storePage'
    $.activityUrl = 'https://jingyun-rc.isvjcloud.com/h5/pages/SignIn/SignIn?id=c0bd4d5a4742f566590e86e424843853&userId=13854617'
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interaction/v2/10023/1001/?shopId=1000002688&activityId=1784832768771424258&shareId=1785589438736211969'
    ...
    $.activityUrl = 'https://lorealjdcampaign-rc.isvjcloud.com/interact/index?activityType=10023&activityId=1789973002922020865&templateId=20210518190900rlqd01&nodeId=101001&prd=crm'
...
class Task extends Env {
    ...
    async getPrizeList(context) {
        if (this.isJinggengAct) {
                ...
                for (const item of description.split("\n")) {
                    ...
                    if (everyDay && everyDay.length > 0) {
                        $.prizeList.push({
                            ...
                            giftType: everyDay[1].includes("积分") ? 9 : everyDay[1].includes("京豆") ? 6 : everyDay[1].includes("优惠券")
                                ? 1 : 7,
                            giftTotal: 99
                        })
                    } else {
            ...
            //jingyun-rc.isvjcloud.com awardTotalQuantity awardSurplusQuantity awardName awardType=[JD_D_COUPON,JD_POINT,JD_E_CARD
            ...
            if ($.prizeList.length === 0 && ["10023"].includes($.activityType)) {
                ...
                if (/(lorealjdcampaign)/.test($.activityUrl)) {
                    //let rule = await this.api('/api/active/getRule', {});
                    ...
                    //1 京豆 3 实物 4 积分
                    //$.prizeList = activityContent.data?.signPiize || [];
            ...
            for (let ele of prizeList) {
                Object.assign(ele, {
                    ...
                    giftName: ele.prizeName, //3 实物 1 京豆
            ...
            if ($.sevenDay) {
                ...
                if (context.act.wxSignActivityGiftBean.hasGiftEveryDay === 'y') {
                    ...
                    context.act.wxSignActivityGiftBean.gift.giftTotal = context.act.wxSignActivityGiftBean.gift.insufficient ? 0
    ...
    async exec() {
        ...
        if (this.isTxzjAct) {
            ...
            if (code === 'success') {
                ...
                this.putMsg(`已签${data.continuity}天`) // : '1', cumulative: '1'
            }
            return
        }
        if (this.isV2Act) {
            ...
            let countSign = calendarData.continuousSignDays //连签
            // let countSign =calendarData.signDays //已签
            ...
            if ($.activityUrl.includes("BudweiserSignIn")) {
                ...
                if (result?.signLog?.lastSignTime && this.formatDate(result.signLog.lastSignTime, 'yyyy-MM-dd') === this.now(
                    "yyyy-MM-dd")) {
                    //已签到
            ...
            if (signLoad.result?.signLog?.lastSignTime && this.formatDate(signLoad.result.signLog.lastSignTime, 'yyyy-MM-dd') === this.now(
                "yyyy-MM-dd")) {
                // let isContinue = signLoad.result?.isContinue//是否连续签到=1
                // let totalCount = signLoad.result?.signLog?.totalCount
                //已签到
        ...
        if (this.isJinggengAct) {
            ...
            //     await this.runCached()
            //     // try {
            //     //     let prize = await this.api("/ql/front/SearchCustomAward", `userId=${$.userId}&actId=${$.activityId}`)
            //     //     if (prize.succ) {
            //     //         for (let record of
            //     //             prize?.qlActLogParams?.resultList?.filter(o => `${o.created?.split(" ")[0]}` === this.now('yyyy-MM-dd'))) {
            //     //             this.putMsg(record.remark)
            //     //             if (record.awardType === "JD_GOODS") {
            //     //                 this.addressId = prize.actLogId;
            //     //                 this.prizeName = record.remark;
            //     //                 await this.saveAddress()
            //     //             }
            //     //         }
            //     //     }
        ...
        if (this.isV1Act) {
            ...
            if (['10002', '10003'].includes($.activityType) && Date.now() >= signInfo.data.signEndTime && Date.now() <= signInfo.data.activityEndTime) {
        ...
        if (!$.__activityContent) {
            let activityContent = await this.api(`sign/${$.sevenDay ? 'sevenDay/' : ''}wx/getActivity`,
        ...
        let signInfo = await this.api(`sign/${$.sevenDay ? 'sevenDay/' : ''}wx/getSignInfo`,
        ...
        if (($.sevenDay && signInfo.isSign === 'n') || (!$.sevenDay && signInfo.signRecord.lastSignDate !== this.now('yyyyMMdd') * 1)) {
            let signUp = await this.api(`/sign/${$.sevenDay ? 'sevenDay/' : ''}wx/signUp`,
                `actId=${$.activityId}&pin=${this.Pin}`)
            if (signUp.isOk) {
                await this.runCached();
                this.__null = true
                countSign++;
        ...
        this.putMsg(`已签${countSign}天`)
        if (!this.__null) {
            let prize = await this.api(`sign/${$.sevenDay ? 'sevenDay/' : ''}wx/getGiftRecords`,
                `venderId=${$.venderId}&pin=${this.Pin}&actId=${$.activityId}`);
            if (prize.isOk) {
    ...
    try {
        ...
        if (countSignArr.length) {
            signDate = this.formatDate(
                addDays(typeof $.actStartTime === 'string' ? $.parseDate($.actStartTime).getTime() : $.actStartTime, max - 1), "yyyy-MM-dd")
        ...
        for (let ele of $.prizeList) {
            if (ele.insufficient) {
                $.msg.push(`    ${ele.everyDay ? '每' : ele.dayNum}天 ${ele.giftName} ${!kv[ele?.giftType] ? ele?.giftType
                    : ''} ${ele.giftTotal}份 已发完`)
                continue
            }
            //true没库存 false 有库存
            if (ele?.dayNum) {
                ...
                if (currentDate === signDate && ele?.dayNum * 1 - max === 1 && (ele?.giftType * 1 === 6 || ele?.giftType * 1 === 7
                    || ele?.giftType * 1 === 13 || ['JD_E_CARD', 'JD_GOODS', 'JD_REDBAG', 'JD_RED_BAG', 'JD_MARKET'].includes(
                        ele?.giftType))) {
                    if (ele?.giftType * 1 === 13 || ele?.giftType * 1 === 7 || ['JD_E_CARD', 'JD_GOODS', 'JD_REDBAG', 'JD_RED_BAG',
                        'JD_MARKET'].includes(ele?.giftType)) {
                        $.msg.push(
                            `    ${ele?.dayNum}天 ${ele.giftName} ${!kv[ele?.giftType] ? ele?.giftType : ''} ${ele?.giftTotal}份 添加定时`)
                    } else if (ele?.giftNum * 1 >= beanLimit) {
                        $.msg.push(
                            `    ${ele?.dayNum}天 ${ele.giftName} ${!kv[ele?.giftType] ? ele?.giftType : ''} ${ele?.giftTotal}份 添加定时`)
                    } else {
                        $.msg.push(
                            `    ${ele?.dayNum}天 ${ele.giftName} ${!kv[ele?.giftType] ? ele?.giftType : ''} ${ele?.giftTotal}份 明日开抢`)
                    }
                    ...
                    $.msg.push(
                        `    ${ele.everyDay ? '每' : ele.dayNum}天 ${ele.giftName} ${!kv[ele?.giftType] ? ele?.giftType
                            : ''} ${ele?.giftTotal}份`)
                }
            } else {
                $.msg.push(
                    `    ${ele.everyDay ? '每' : ele.dayNum}天 ${ele.giftName} ${!kv[ele?.giftType] ? ele?.giftType : ''} ${ele?.giftTotal
                    || '-'}份`)
            }
        }
    } catch (e) {
