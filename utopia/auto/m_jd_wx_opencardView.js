    ...
    async exec() {
        ...
        if (this.isJinggengjcqAct) {
            ...
            for (let shop of $.shopList?.data?.data) {
                let viewShop = await this.api(
                    `/dm/front/${jinggengcjqPathType}/mission/completeMission?open_id=&mix_nick=${this.buyerNick
                    || ""}&user_id=10299171`,
                    {
        ...
        if ($.activityUrl.includes("/m/unite/") || $.activityUrl.includes("joinCommon")) {
            ...
            for (let ele of data.mainActive?.settings || []) {
                const startVisitTask = await this.api('dingzhi/common/visitTask/startVisitTask',
                ...
                const saveTask = await this.api('dingzhi/shop/league/saveTask',
                    `activityId=${$.activityId}&pin=${this.Pin}&actorUuid=${this.actorUuid}&taskUuid=${taskUuid}&shareUuid=${this.shareUuid
