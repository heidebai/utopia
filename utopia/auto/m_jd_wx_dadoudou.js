...
class Task extends Env {
    ...
    async exec() {
        ...
        let activityContent = await this.api('wxgame/activityContent',
            `activityId=${$.activityId}&pin=${this.Pin}&pinImg=${encodeURIComponent(
                $.attrTouXiang)}&nick=${this.ptpin}&cjyxPin=&cjhyPin=&shareUuid=${$.shareId || ''}`);
                ...
                try {
                    if (ele.taskId === 'followsku') {
                        ...
                        for (let i = 0; i < followskuData.data.length && i < ele.maxNeed; i++) {
                            ...
                            await this.api('wxgame/doTask',
                                `activityId=${$.activityId}&pin=${this.Pin}&taskId=${ele.taskId}&param=${fsku.skuId}`)
                        }
                        ...
                        for (let i = 0; i < add2cartData.data.length && i < ele.maxNeed; i++) {
                            ...
                            await this.api('wxgame/doTask',
                                `activityId=${$.activityId}&pin=${this.Pin}&taskId=${ele.taskId}&param=${asku.skuId}`)
                        }
                    } else {
                        let result = await this.api('wxgame/doTask',
                            `activityId=${$.activityId}&pin=${this.Pin}&taskId=${ele.taskId}&param=`);
                    }
                } catch (e) {
                    this.log(e)
                }
        ...
        for (let i = 0; i < Math.min(tasks?.data?.chance, 7); i++) {
            try {
                ...
                await this.api('wxgame/game/end',
                ...
                let reqTime =  Date.now()
                let luckyDraw = await this.api('wxgame/game/luckyDraw',
                    `activityId=${$.activityId}&pin=${this.Pin}&score=${gameScore}&gameId=${gameId}&reqtime=${reqTime}&sign=${CryptoJS.MD5(`${gameId},${reqTime},0eed6538f6e84b754ad2ab95b45c54f8`).toString()}`)
                if (luckyDraw.result) {
