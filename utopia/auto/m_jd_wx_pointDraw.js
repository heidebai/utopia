...
const $ = new Env('M积分兑换');//抽奖
$.activityUrl = decodeURIComponent(process.argv.splice(2)?.[0] || process.env.M_WX_POINT_DRAW_URL);
if (mode) {
    $.activityUrl = 'https://cjhy-isv.isvjcloud.com/mc/wxPointShopView/pointExgShiWu?venderId=1000002984&giftId=f4db0acf4bec427ca1bd66308b4fd67f&giftType=3'
    ...
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10079&activityId=1752946530495176706&templateId=202209051007jfdh&nodeId=101001&prd=cjwx'
    ...
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10079&activityId=1760541959408373762&templateId=202209051007jfdh&nodeId=101001&prd=cjwx'
    ...
    $.activityUrl = 'https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10079&activityId=1763115019875713025&templateId=202209051007jfdh&nodeId=101001&prd=cjwx'
    // $.activityUrl = 'https://jingyun-rc.isvjcloud.com/h5/pages/exchangeJDmarket/exchangeJDmarket?id=812eb8afd8626a5731147d5efff73a8c&userId=1000116702'
    $.runMode = "master";
    $.cookieTypes = ['master']
}
//屏蔽实物的店铺
$.notPointDrawList = process.env?.M_WX_NOT_POINT_DRAW_LIST?.split(/[@,&|]/).join("|") || "";//屏蔽的店铺
//屏蔽豆子的店铺
...
class Task extends Env {
    ...
    async exec() {
        ...
        if (this.isHdbAct) {
            ...
            //let tow = await this.api(`/front/activity/load${fn}Awards`, {});
        ...
        if (this.isV2Act) {
            ...
            let {
                ...
                rightsNum,
                rules,
                exchangeCycleStartTime,
                exchangeCycleEndTime,
                exchangePointNum
            } = result?.data;
                        ...
                        try {
                            if (item.tel.includes(addr.phone)) {
                                ...
                                if (receivePrize?.data?.successNum > 0) {
                                    await fs.appendFileSync("gifts.csv",
                                        `${$.now()},${rightsName},${this.ptpin},${addr.phone},${addr.address},${$.name},${this.super?.shopName
                                        || '未知'},${$.activityUrl}\n`)
        ...
        ///mc/hb/selectHbForC
        //{giftId: giftId, venderId: venderId, buyerPin: buyerMixPin}
        //https://cjhy-isv.isvjcloud.com/mc/wxPointShopView/pointExgHb?venderId=1000101918&giftType=12&giftId=9852cdbb893e49c9a47effd00ab08417&sid=c0021fea54dd860ca35b7d32016e681w&un_area=1_2809_51230_0
        let activityContent;
        if ($.activityUrl.includes('pointExgECard')) {
            activityContent = await this.api('mc/equity/selectEquityForC',
                `giftId=${$.activityId}&venderId=${$.venderId}&buyerPin=${this.Pin}`);
            ...
            activityContent = await this.api('mc/beans/selectBeansForC',
        ...
        if ($.activityUrl.includes('pointExgECard')) {
            for (let i = 0; i < 10; i++) {
                let prize = await this.api('/mc/wxPointShop/exgECard',
        ...
        if ($.activityUrl.includes('pointExgShiWu')) {
            ...
            if (selectAddressList.ok && selectAddressList.count >= 1) {
                //this.addressId = selectAddressList.data[0].addressId
                for (let ele of selectAddressList.data) {
                    let kk = await this.api('mc/wxPointShop/deleteAddress',
                        `venderId=${$.venderId}&buyerPin=${this.Pin}&addressId=${ele.addressId}`);
                    console.log(kk);
                }
            }
//https://cjhy-isv.isvjcloud.com/mc/wxPointShop/deleteAddress
            ...
            //@formatter:off
            let saveAddress = await this.api('/mc/wxPointShop/saveAddress', `buyerPin=${this.Pin}&venderId=${$.venderId}&receiver=${encodeURIComponent(addrInfo.receiver)}&receiverPhone=${addrInfo.phone}&province=${encodeURIComponent(addrInfo.province)}&city=${encodeURIComponent(addrInfo.city)}&county=${encodeURIComponent(addrInfo.county)}&address=${encodeURIComponent(addrInfo.address)}`);
            //@formatter:on
            ...
            if (Array.isArray($.mcShiWu) && $.mcShiWu.length) {
                date = new Date();
                var hours = $.mcShiWu.takeBeginTime.substring(0, 2); // 提取前两个字符
                ...
                if ((start - now) < 60 && (start - now) > 0) {
                    //$.log(`脚本时间 ${(new Date()).Format("yyyy-MM-dd hh:mm:ss")}`);
                    //提前1秒
            ...
            while (count < 10) {
                count++;
                let prize = await this.api('/mc/wxPointShop/exgShiWu', `buyerPin=${this.Pin}&buyerNick=${encodeURIComponent(
                    $.nickname)}&giftId=${$.activityId}&venderId=${$.venderId}&addressId=${this.addressId}`)
                if (prize.result) {
                    this.putMsg(`兑换成功`);
                    await fs.appendFileSync("gifts.csv",
                        `${$.now()},${$.giftName},${$.username},${addrInfo.phone},${addrInfo.address},${$.name},${$.shopName},${$.activityUrl}\n`)
                    let selectAddressList = await this.api('mc/wxPointShop/selectAddressList',
                        `venderId=${$.venderId}&buyerPin=${this.Pin}`);
                    if (selectAddressList.ok && selectAddressList.count >= 1) {
                        //this.addressId = selectAddressList.data[0].addressId
                        for (let ele of selectAddressList.data) {
                            let kk = await this.api('mc/wxPointShop/deleteAddress',
        ...
        if ($.mcConfig.timeLimitJson != null) {
            ...
            date = new Date();
            var hours = startTime[0].timeLimitStartTime.substring(0, 2); // 提取前两个字符
        ...
        while (count < 10) {
            count++;
            let prize = await this.api('mc/wxPointShop/exgBeans',
            ...
            if (prize.errorMessage.includes('积分不足') || prize.errorMessage.includes('每人每日兑换最大上限')
                || prize.errorMessage.includes(
                    '等级不符')) {
                this.putMsg(`${prize.errorMessage}`);
                break
            }
...
$.after = async function () {
    //M_WX_POINT_DRAW_URL="https://cjhy-isv.isvjcloud.com/mc/wxPointShopView/pointExgBeans?venderId=1000011225&giftType=4&giftId=8011b028b1f244cba8d500c430667911&ad_od=share"
