...
if (mode) {
    $.activityUrl = 'https://lzdz1-isv.isvjcloud.com/m/unite/8252676/dzlh0001/?activityId=a20d800cb00c44f6b8b60e4f8f5827d9&venderId=12160232&shareUuid=a3c74c906e194ffc90bf2a89cbb7deae&adsource=null'
}
...
class Task extends Env {
    ...
    async exec() {
        ...
        if ($.isSzxyunAct) {
            ...
            let {code, message, data: active} = await this.api("/webc/unionOpen/active", {
                activeId: $.activityId, shareId: this.shareUuid || null
            })
                ...
                while (x-- > 0) {
                    try {
                        let {data: lottery} = await this.api("/webc/unionOpen/lottery", {
                            lotteryForm: 1, activeId: $.activityId, joinId: this.joinId
                        })
        ...
        if ($.isJinggengjcqAct) {
            ...
            if (this.shareUuid) {
                ...
                if (inviteRelation.errorMessage.includes("关系绑定成功")) {
                    //+1
                    shareCodes.filter(o => o.shareUuid === this.shareUuid).forEach(o => {
                        o.count++
                    })
                } else {
                    if (!this.isMaster()) {
            ...
            try {
                if (!load.data.data.missionCustomer.hasCollectShop) {
                    await this.api(`/dm/front/${jinggengcjqPathType}/mission/completeMission?open_id=&mix_nick=${this.buyerNick
                        || ""}&user_id=10299171`,
        ...
        if (this.isMaster()) {
            ...
            if ($.activityId.includes("shop")) {
                try {
                    let {data: getShareRecord} = await this.api('/dingzhi/taskact/common/getShareRecord',
        ...
        if ($.activityId.includes("shop")) {
            this.shareUuid = shareCodes.sort(function (a, b) {
    ...
    async helpShare() {
        let {data} = await this.api("/webc/unionOpen/share", {
            activeId: $.activityId, joinId: this.joinId, shareId: this.shareUuid || null
        })
