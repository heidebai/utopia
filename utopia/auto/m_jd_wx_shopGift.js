...
class Task extends Env {
    ...
    async exec() {
        ...
        if (this.isTxzjAct) {
           ...
           // let {code, data} = await this.api("/shop_gift/send_prize", `pid=${$.activityId}`);
            if (code === 'success') {
                await this.runCached();
        ...
        if (this.isV1Act) {
            let prize = await this.api('/api/shopGift/drawShopGift',
                {'flag': true, 'memberUser': 0, 'name': "", 'visitor': "", 'position': ""});
            if (prize.data) {
                this.putMsg(`领取成功`);
                await this.runCached();
            } else {
        ...
        if (this.isGzslAct) {
            ...
            let prize = await this.api(
                `wuxian/user/getShopGiftPrize/${prizeId}?wxToken=${this.isvToken}`, {
                    'token': this.isvToken,
        ...
        let activityContent = await this.api('wxShopGift/activityContent',
            `activityId=${$.activityId}&buyerPin=${this.Pin}`);
        ...
        let prize = await this.api('wxShopGift/draw',
            `activityId=${$.activityId}&buyerPin=${this.Pin}&hasFollow=false&accessType=app`);
        if (prize.result) {
            this.putMsg('领取成功');
            await this.runCached();
        }
