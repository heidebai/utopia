...
class Task extends Env {
    ...
    async exec() {
        ...
        let activityContent = await this.api(`mc/wxMcLevelAndBirthGifts/activityContent`,
            `activityId=${$.activityId}&pin=${this.Pin}&level=1`);
        ...
        if (isReceived === 1) {
            this.putMsg("已领过");
            let drawBirthdayRecord = await this.api('mc/wxMcLevelAndBirthGifts/getDrawBirthdayRecord',
        ...
        if ($.activityType === 103) {
            ...
            if (!birthInfo.result) {
                this.log("去填生日")
                for (let i = 0; i < 5; i++) {
                    var newVar = await this.api('mc/wxMcLevelAndBirthGifts/saveBirthDay',
            ...
            draw = await this.api('mc/wxMcLevelAndBirthGifts/sendBirthGifts',
            ...
            draw = await this.api(`mc/wxMcLevelAndBirthGifts/sendLevelGifts`,
                `venderId=${$.venderId}&activityId=${$.activityId}&pin=${this.Pin}&level=${level}`);
            try {
