...
$.version = "v1.0.0"
if (mode) {
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10024&activityId=1727250072363245570&templateId=7f4116e9-34b2-4f6d-9684-d7ca7c4b3d69&nodeId=101001&prd=cjwx"
    $.activityUrl = "https://jinggeng-isv.isvjcloud.com/ql/front/showCart?id=9e8080bc8a4e3345018a5020d6a427da&user_id=190205"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10024&activityId=1732952656497049602&templateId=c2af9114-85d5-4c31-ab61-201f370f0f2e&nodeId=101001&prd=cjwx"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10024&activityId=1734760293945540610&templateId=c2af9114-85d5-4c31-ab61-201f370f0f2e&nodeId=101001&prd=cjwx"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10024&activityId=1737353655227850754&templateId=c2af9114-85d5-4c31-ab61-201f370f0f2e&nodeId=101001&prd=cjwx"
    ...
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interaction/v1/index?activityType=10024&activityId=1727564379523313666&templateId=20210518190900jgyl01&nodeId=101001&prd=crm"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/wxCollectionActivity/activity2/5aa3b1a806fe4f978d659c577cc33156?activityId=5aa3b1a806fe4f978d659c577cc33156"
    $.activityUrl = "https://cjhy-isv.isvjcloud.com/wxCollectionActivity/activity?activityId=ae7ff71c331046ddabdc66b1af5e47f6"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interaction/v1/index?activityType=10024&templateId=e0fa23cd-9120-47ff-ad44-dde6c92bfbc6&activityId=1734402298685677569&nodeId=101001&prd=crm&shopid=1000002836 "
    $.activityUrl = "https://lorealjdcampaign-rc.isvjcloud.com/interact/index?activityType=10024&activityId=1760252728879730690&templateId=20210518190900jgyl01&nodeId=101001&prd=crm "
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10024&activityId=1764846516851310594&templateId=c2af9114-85d5-4c31-ab61-201f370f0f2e&nodeId=101001&prd=cjwx "
    ...
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interaction/v1/index?activityType=10024&templateId=e0fa23cd-9120-47ff-ad44-dde6c92bfbc6&activityId=1762754385111003137&nodeId=101001&prd=crm&shopid=1000003039"
    $.activityUrl = "https://cjhy-isv.isvjcloud.com/wxCollectionActivity/activity?activityId=77da0a08c39347c7ad4e718c3afc78f0"
    $.activityUrl = "https://lzkj-isv.isvjcloud.com/prod/cc/interactsaas/index?activityType=10024&activityId=1779764425579671554&templateId=c2af9114-85d5-4c31-ab61-201f370f0f2e&nodeId=101001&prd=cjwx"
...
class Task extends Env {
    ...
    async exec() {
        ...
        let context = await this.login(
            {fn: $.isJinggengAct ? `ql/front/showCart` : `wuxian/user/getGoodsGiftActivity/${$.activityId}`});
        if (['txzj-isv.isvjcloud.com'].includes($.domain)) {
            ...
            if (code === 'success') {
                ...
                if (['coupon'].includes(data.prize_title.type)) {
                    ...
                    this.putMsg(`${msg} ${data?.prize_title?.type}`) // : '1', cumulative: '1'
                    $.limit++
                }
            }
            return
        ...
        if (this.isGzslAct) {
            ...
            let prize = await this.api(`wuxian/user/getGoodsGiftPrizeAndWare/${prizeId}?wxToken=${this.isvToken}`, {
                'token': this.isvToken, 'wxToken': this.isvToken, 'activityId': prizeId, 'source': "01"
            });
        ...
        if (this.isV1Act) {
            ...
            let skuIds = $.randomArray(activity.data.addWares.skuInfoVO.map(vo => vo.skuId + ""), $.needCollectionSize)

            let count = 1;
            //是否需要开卡
            for (let i = 0; i < skuIds.length; i++, count++) {
                let todo = await this.api('/api/task/addSku/toDo', {
                    'taskId': taskId, 'skuId': skuIds[i]
                })
        ...
        if (oneKeyAddCart) {
            let carInfo = await this.api('wxCollectionActivity/oneKeyAddCart',
            ...
            if (carInfo.result && carInfo.data) {
                ...
                if (carInfo.errorMessage.includes("店铺会员")) {
                    if (drawInfo.type === 6) {
                        ...
                        carInfo = await this.api('wxCollectionActivity/oneKeyAddCart',
                            `activityId=${$.activityId}&pin=${this.Pin}&productIds=${encodeURIComponent(JSON.stringify(skuIds))}`)
                        if (carInfo.result) {
            ...
            let counter = 0;
            for (let i = 0; i < skuIds.length; i++) {
                let carInfo = await this.api(`wxCollectionActivity/${$.activityType * 1 === 5 ? 'collection' : 'addCart'}`,
                ...
                if (carInfo.result) {
                    ...
                    // if (skuIds.length - i < needCollectionSize - currAddNum) {
                    //     this.putMsg(`剩余商品不足退出`)
                    //     break
                    // }
                    if (carInfo.errorMessage.includes("店铺会员")) {
                        if (drawInfo.type === 6) {
        ...
        // if (!isAddSuccess) {
        //     try {
        //         await this.api(`wxCollectionActivity/addCart`, `activityId=${$.activityId}&pin=${this.Pin}&productId=${skuIds[0]}`)
        //         isAddSuccess = true;
        //     } catch (e) {
        //     }
        // }
...
$.after = async function () {
    ...
    try {
        ...
        for (let ele of $.prizeList) {
            this.msg.push(`    加${$.needCollectionSize || addNum?.[2]}件,${ele.name || ele.prizeName || ele.equityName
            || ele.awardName},共${ele.leftNum || prizeCount?.[2] || '未知'}份`)
        }
    } catch (e) {
        console.log(e)
    }
